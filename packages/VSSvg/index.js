import AddIcon from './AddIcon';
import AlertFillIcon from './AlertFillIcon';
import CallFillIcon from './CallFillIcon';
import CallIcon from './CallIcon';
import ChartFillIcon from './ChartFillIcon';
import ChartIcon from './ChartIcon';
import CheckmarkCircleIcon from './CheckmarkCircleIcon';
import CheckmarkCircleFillIcon from './CheckmarkCircleFillIcon';
import CircleFillIcon from './CircleFillIcon';
import CloseIcon from './CloseIcon';
import CoalFillIcon from './CoalFillIcon';
import DownChevron from './DownChevron';
import EyeFillIcon from './EyeFillIcon';
import EyeIcon from './EyeIcon';
import FuelTankFillIcon from './FuelTankFillIcon';
import GasFillIcon from './GasFillIcon';
import GroupFillIcon from './GroupFillIcon';
import HomeFillIcon from './HomeFillIcon';
import HomeIcon from './HomeIcon';
import InfoFillIcon from './InfoFillIcon';
import LeftChevron from './LeftChevron';
import LoadingSun from './LoadingSun';
import NoViewFillIcon from './NoViewFillIcon';
import NoViewIcon from './NoViewIcon';
import PencilFillIcon from './PencilFillIcon';
import PineTreeFillIcon from './PineTreeFillIcon';
import PropaneFillIcon from './PropaneFillIcon';
import RecycleFillIcon from './RecycleFillIcon';
import RightChevron from './RightChevron';
import SendIcon from './SendIcon';
import SettingsFillIcon from './SettingsFillIcon';
import SettingsIcon from './SettingsIcon';
import SignOutIcon from './SignOutIcon';
import ThumbprintIcon from './ThumbprintIcon';
import TrashFillIcon from './TrashFillIcon';
import WarningFillIcon from './WarningFillIcon';
import WarningIcon from './WarningIcon';
import VSLogo from './VSLogo';

module.exports = {
  AddIcon,
  AlertFillIcon,
  CallFillIcon,
  CallIcon,
  ChartFillIcon,
  ChartIcon,
  CheckmarkCircleIcon,
  CheckmarkCircleFillIcon,
  CircleFillIcon,
  CloseIcon,
  CoalFillIcon,
  DownChevron,
  EyeFillIcon,
  EyeIcon,
  FuelTankFillIcon,
  GasFillIcon,
  GroupFillIcon,
  HomeFillIcon,
  HomeIcon,
  InfoFillIcon,
  LeftChevron,
  LoadingSun,
  NoViewFillIcon,
  NoViewIcon,
  PencilFillIcon,
  PineTreeFillIcon,
  PropaneFillIcon,
  RecycleFillIcon,
  RightChevron,
  SendIcon,
  SettingsFillIcon,
  SettingsIcon,
  SignOutIcon,
  ThumbprintIcon,
  TrashFillIcon,
  WarningFillIcon,
  WarningIcon,
  VSLogo,
};
